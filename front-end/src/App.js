...
export default class Site extends Component {
  ...
  loadResults(request) {
    console.log(request);
    axios
      .get(`http://localhost:8888/api/nlp?request=${request}`)
      .then(response => {
        this.setState({
        ...
        // TODO
        // 1) map above values to the request object of https://www.apartments.com/services/search/
        // 2) parse out the pinState object to get the listingkeys also will need the searchCriteria object
        // 3) iterate through listingkeys and call https://www.apartments.com/services/property/infoCardData with listingkey and searchCriteria
      })
      .catch(error => {
        console.log(error);
      });
  ...
  render() {
    return (
      <div>
        ...
        <Table
          min_sqft={this.state.min_sqft}
          max_sqft={this.state.max_sqft}
          min_bdrm={this.state.min_bdrm}
          max_bdrm={this.state.max_bdrm}
